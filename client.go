...
const (
	...
	BASE_URL       = `BASE_URL`
	ILMN_PROJECT   = `X-ILMN-Project`
)

func requiredKeys() []string {
	return []string{
		AUTH_TOKEN, ILMN_DOMAIN, ILMN_WORKGROUP, ILMN_PROJECT,
	}
}

func getTimeoutSeconds(cfg map[string]string) int {
	//from config
	if s, ok := cfg[`GLACIERGO_TIMEOUT_SECOND`]; ok {
		if n, err := strconv.Atoi(s); err == nil && n > 0 {
			return n
		}
	}
	//from env
	if s := os.Getenv(`GLACIERGO_TIMEOUT_SECOND`); s != "" {
		if n, err := strconv.Atoi(s); err == nil && n > 0 {
			return n
		}
	}
